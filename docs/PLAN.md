...
- [ ] **Replace emoji buddy with real art** — the one remaining art task. Commission a small
      sprite set (idle / napping / playing, cat + dog) from an artist on Fiverr or itch.io;
      budget $100–400. The app is fully functional without it, so this is not a blocker.
- [ ] Nice-to-have: Live Activity on lock screen / Dynamic Island for the running timer.
      Needs a Widget Extension target — worth adding once the app builds cleanly on your Mac.
...
- [ ] More buddies (bunny, hamster) — possible cosmetic in-app purchase
- [ ] Buddy levels up / unlocks accessories with completed sessions
- [ ] Home screen widget showing today's paw prints
