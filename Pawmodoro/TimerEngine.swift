...
final class TimerEngine {
    ...
    /// The season letters, and the year kept: the almanac's own records — and
    /// every world event written down as it happens. See `Chronicle`.
    ...
    init(
        ...
        if let raw = LaunchOptions.forcedFind, let find = Trinket(rawValue: raw) {
    ...
    func resolveJourneys(now: Date = Date()) {
        for journey in travels.due(now: now, selected: settings.buddy) {
            ...
            let letter = Letter(
                ...
                keepsake: trinket.rawValue, reportedSpecies: gap?.rawValue
            )
            drawer.add(trinket, place: place, finder: buddy, on: now)
            if let gap {
                setBias(SightingBias(
    ...
    /// over what it was holding — berries to the sill, a trinket to the
    ...
    private func bankDrift(from start: Date, elapsed: TimeInterval) {
        ...
        for (minutes, endedAt) in zip(banking.minutes, dates) {
            log.add(minutes: minutes, place: settings.place,
                    buddy: settings.buddy, endedAt: endedAt)
        }
        ...
        if let sighting {
            ...
            // The pale coat marks the sketch forever — a memory mark, never a
        ...
        if let dream {
            ...
            // And a kept dream drops a seed by the windowsill — the garden
            // grows what the dreams dreamed.
            garden.offerSeed(from: dream)
        }
        // The session's warm epilogue: the world sets out one snack for the
        // buddy. The sill holds one at most, so this never stacks.
    ...
    func syncAfterWake() {
        ...
        // and an app re-entered on a new day gets its doorstep moment. Ahead
        // of the drift branch below, because a day still turns over while
        // somebody is drifting through it.
        pantry.sweep()
        resolveNightVisit()
        doorstep.arrive(
